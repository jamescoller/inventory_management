...
- [x] **Hierarchical location search (nav bar + `/search/`, parent → descendants, `LOC-{id}`)** — ✅ **Done (PR #123):** `Location.descendant_ids()` + `_expanded_location_ids()`; navbar and `/search/` both expand a matched container to its subtree, and a typed `LOC-<id>` filters. **Deferred (needs a UX call):** whether a *typed* `LOC-<id>` in the navbar should jump to the audit console (as a *scanned* `LOC-` barcode does) rather than filter. Search inventory by location, expanding containers to their whole subtree.
